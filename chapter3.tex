...
\tablecaption{SpARCS $z\sim1.6$ cluster sample\label{tbl-clusters}}
\tablewidth{0pt}
\tablehead{
\colhead{Cluster}
& \colhead{R.A.}
...
Altogether there are 113 spectroscopically-confirmed members across the four clusters in this sample (see Table \ref{tbl-clusters}).
...
\caption{Rest-frame \textit{U}-\textit{V} versus absolute \textit{J} magnitude diagram for photometric-redshift-selected cluster members of the four clusters in the sample (see Table \ref{tbl-clusters}).
...
\caption{Dust-corrected rest-frame \textit{U-V} versus absolute \textit{J} magnitude diagram for the four clusters in our sample.
...
\begin{figure}[h!]
\centering \includegraphics[width=0.5\textwidth]{figures/c2/ellipses_clustering.pdf}
\caption{Classification of star-forming, intermediate, and quiescent galaxies.
...
Note that even after 6 Gyr of constantly-integrated star formation, remains fully within the star-forming ellipse, and only leaves it upon quenching.
\label{fig-ellipses}}
\end{figure}
...
\tabletypesize{\scriptsize}
\tablecaption{Quenching Timescales\label{tbl-results}}
\tablecolumns{9}
\tablewidth{0pt}
\tablehead{
\colhead{Cluster} \vspace{-0.4cm} & & & & & & \colhead{$t_D$} & \colhead{$t_F$} & \colhead{$t_Q$} \\ \vspace{-0.4cm}
& \colhead{N$^{a}$} & \colhead{$\bar{z}$} & \colhead{R$^b$} & \colhead{G$^b$} & \colhead{B$^b$} & & & \\
...
We then use the total number of red, blue, and green galaxies to measure a quenching timescale according to Equations~\eqref{eq-model-first}~--~\eqref{eq-model-last}, finding $t_Q=$\gresult Gyr at $z\sim1$.
...
\citet{Balogh:2016aa} also finds a quenching time of $7.0 \pm 0.5$ Gyr in SDSS groups, $2.8 \pm 0.5$ Gyr in GEEC2 groups, and $1.5 \pm 0.5$ in the GCLASS cluster sample.
...
% \acknowledgments{G.W. acknowledges financial support for this work from NSF grant AST-1517863 and from NASA through programs GO-13306, GO-13677, GO-13747 \& GO-13845/14327 from the Space Telescope Science Institute, which is operated by AURA, Inc., under NASA contract NAS 5-26555.
% JN is supported by Universidad Andres Bello internal research grant DI-18-17/RG.
% Support for MCC was provided in part by NSF grant AST-1518257 as well as by NASA through grants AR-13242 and AR-14289 from the Space Telescope Science Institute, which is operated by the Association of Universities for Research in Astronomy, Inc., under NASA contract NAS 5-26555.
% PC acknowledges the support of the FONDECYT postdoctoral research grant no 3160375.
...
\begin{figure}[h!]
\centering \includegraphics[width=0.5\textwidth]{figures/c2/appendix/sfh.pdf}
\caption{Model star formation rate of a galaxy as a function of time relative to its accretion by the cluster.
...
\caption{Cluster mass accretion rate $\mathrm{d}M/\mathrm{d}z$ as a function of redshift, for a cluster observed at redshift $z_c$.
...
\caption{Modeled evolution of the fractions $\frac{B}{G+R}$ and $\frac{G}{R}$.
...
\begin{figure}
\centering \includegraphics[width=0.5\textwidth]{figures/c2/appendix/shechter.pdf}
\caption{Evolution of the field quiescent fraction with redshift.
...
\caption{Left panel: Rest-frame \textit{U-V} versus \textit{V-J} color-color diagram for all cluster members in the high-redshift sample.
...
\tabletypesize{\scriptsize}
\tablecaption{Quenching Timescales from UVJ Selection\label{tbl-UVJ}}
\tablecolumns{9}
\tablewidth{0pt}
\tablehead{
